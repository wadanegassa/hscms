  ...
  return (
    <div className="fade-in" style={{ paddingBottom: '2rem' }}>
      <div className="flex-between mb-8" style={{ alignItems: 'flex-end' }}>
        <div>
          <h2 className="page-title" style={{ marginBottom: '0.5rem', fontSize: '2rem' }}>💳 Record Repayment</h2>
          <p style={{ color: 'var(--text-secondary)', fontSize: '1rem', margin: 0 }}>
            Search for a member and record their loan repayment.
          </p>
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: member ? '1fr 1fr' : '1fr', gap: '2rem', alignItems: 'start' }}>
        <div className="glass-card scale-up" style={{ padding: '2.5rem' }}>
          ...
          <div style={{ opacity: member ? 1 : 0.5, pointerEvents: member ? 'auto' : 'none', transition: 'all 0.3s ease' }}>
            <h4 className="section-title" style={{ fontSize: '1.125rem', marginBottom: '1.25rem' }}>2. Repayment Details</h4>

            {member && loans.length === 0 ? (
              <div style={{ padding: '1.5rem', background: 'rgba(239, 68, 68, 0.05)', border: '1px solid rgba(239, 68, 68, 0.1)', color: 'var(--danger)', borderRadius: '16px', textAlign: 'center' }}>
                <p style={{ fontWeight: 700, margin: 0 }}>No active loans found for this member.</p>
              </div>
            ) : (
              <form onSubmit={handleSubmit}>
                <div className="input-group">
                  <label className="label">Select Active Loan</label>
                  <select
                  ...
                  >
                    ...
                    {loans.map(loan => (
                      <option key={loan._id} value={loan._id}>
                        {loan.purpose} - {loan.amount.toLocaleString()} ETB ({loan.status})
                      </option>
                    ))}
                  </select>
                </div>
                ...
                <div className="input-group">
                  <label className="label">Repayment Amount (ETB)</label>
          ...
          <div className="glass-card scale-up" style={{ padding: '2.5rem', border: '1px solid var(--info)' }}>
            ...
            <div style={{
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
                <span style={{ color: 'var(--text-muted)', fontWeight: 600, fontSize: '0.875rem' }}>Active Loans</span>
                <span style={{ color: 'var(--text-primary)', fontWeight: 700 }}>{loans.length}</span>
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <span style={{ color: 'var(--text-muted)', fontWeight: 600, fontSize: '0.875rem' }}>Member Since</span>
                <span style={{ color: 'var(--text-primary)', fontWeight: 700 }}>{new Date(member.createdAt).toLocaleDateString()}</span>
              </div>
            </div>

            <div style={{ padding: '1.5rem', borderRadius: '20px', background: 'rgba(59, 130, 246, 0.05)', border: '1px solid rgba(59, 130, 246, 0.1)' }}>
              <div style={{ textAlign: 'center' }}>
                <div style={{ fontSize: '0.75rem', fontWeight: 700, color: 'var(--info)', textTransform: 'uppercase', marginBottom: '0.5rem' }}>Total Borrowed</div>
                <div style={{ fontSize: '2rem', fontWeight: 800, color: 'var(--text-primary)' }}>
                  {(loans || []).reduce((acc, l) => acc + (l.amount || 0), 0).toLocaleString()} <span style={{ fontSize: '1rem', color: 'var(--text-muted)' }}>ETB</span>
                </div>
              </div>
            </div>
          </div>
